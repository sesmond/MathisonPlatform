...
MIN_EDGE_HEIGHT = 2  # 切边的最小高度
MAX_EDGE_HEIGHT = 10  # 切边的最大高度
...
def save_image(original_name, type, dst_image, json_data):
    prefix, subfix = os.path.splitext(original_name)
    f_name = os.path.join(enhance_path, prefix + "_" + type + subfix)
    # print("保存图像：",f_name)
    cv2.imwrite(f_name, dst_image)
    # 保存文本标签
    enhance_json_path = os.path.join(enhance_path, prefix + "_" + type + '.json')
    with open(enhance_json_path, "w", encoding='utf-8') as g:
        json.dump(json_data, g, indent=2, sort_keys=True, ensure_ascii=False)
...
def cut_edge(image, label, file_name):
    ...
    cv2.imwrite(os.path.join(enhance_path + file_name[:-4] + '_up' + '.jpg'), img_up)
    with open(enhance_path + file_name[:-4] + '_up' + '.txt', "w", encoding='utf-8') as f:
        f.write(str(label))

    # 切下边，坐标不变
    img_down = image[0:h - e, 0:w]
    cv2.imwrite(os.path.join(enhance_path + file_name[:-4] + '_down' + '.jpg'), img_down)
    with open(enhance_path + file_name[:-4] + '_down' + '.txt', "w", encoding='utf-8') as f:
        f.write(str(label))

    # 切右边，坐标不变
    img_right = image[0:h, 0:w - e]
    cv2.imwrite(os.path.join(enhance_path + file_name[:-4] + '_right' + '.jpg'), img_right)
    with open(enhance_path + file_name[:-4] + '_right' + '.txt', "w", encoding='utf-8') as f:
        f.write(str(label))

    # 切左边
    img_left = image[0:h, e:w]
    cv2.imwrite(os.path.join(enhance_path + file_name[:-4] + '_left' + '.jpg'), img_left)
    with open(enhance_path + file_name[:-4] + '_left' + '.txt', "w", encoding='utf-8') as f:
        f.write(str(label))
...
## [[982, 23], [947, 23], [947, 6], [982, 6], '宜信']-->982,23,947,23,947,6,982,6,宜信
def format_convert(label):
    pos = label[:4]
    x1 = pos[0][0]
    y1 = pos[0][1]
    x2 = pos[1][0]
    ...
    y4 = pos[3][1]
    txt = label[4]
    label = x1, y1, x2, y2, x3, y3, x4, y4, txt
...
def do_folder(p_no, folder, image_list):
    ...
    for file_name in image_list:
        ...
        if ext != ".jpg":
            ...
            with open(origin_path + file_name[:-4] + '.json', "r") as f:
                json_data = json.load(f)
                label = json_data['']

            save_image(file_name, "原图", img, json_data)
            enhance_all_with_save(img, file_name, json_data)
...
origin_path = "data/summary/card_rotate/"
enhance_path = "data/summary/card_enhance/"
...
if __name__ == "__main__":
    ...
    image_all = os.listdir(origin_path)
    ...
    for img_list in file_list_arr:
        ...
        pool.apply_async(do_folder, args=(p_no, origin_path, img_list))
        p_no += 1
    pool.close()
    pool.join()
